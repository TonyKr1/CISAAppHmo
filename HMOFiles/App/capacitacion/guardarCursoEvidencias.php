<?php
    ...
    $evidencias = json_decode($_POST['evidencias']);
    ...
    //     //sqlsrv_free_stmt($stmt_Insert_datos);
        ...
        foreach($evidencias as $objevidencias){
            $parametros = '';
            $parametros = array(
                array($id_cedulaPadre),
                array($FK_DatosGeneralesCurso),
                array($objevidencias->evidencia),
                array($objevidencias->fecha)
            );

            $C_Insert_check = "INSERT INTO CAP_Evidencias(FK_CursosHeader, FK_DatosGenerales, Evidencia, fecha) 
            VALUES (?, ?, ?, ?);";
